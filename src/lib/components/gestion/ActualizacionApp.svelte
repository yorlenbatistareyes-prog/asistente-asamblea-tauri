...
<div style="display: flex; flex-direction: column; gap: 30px; padding-bottom: 20px; max-width: 800px;">
    <Panel padding="30px">
        <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 15px;">
            <div style="width: 64px; height: 64px; background: rgba(37, 99, 235, 0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: var(--primary);">
                <DownloadCloud size={32} />
            </div>
            <div>
                <h2 style="margin: 0 0 5px 0; font-size: 20px; color: var(--text-main);">Actualizaciones de RAssembly</h2>
                <p style="margin: 0; color: var(--text-secondary); font-size: 14px;">Versión instalada: <strong>v{#if versionReal}{versionReal}{:else}...{/if}</strong></p>
            </div>
            
            <p style="color: var(--text-secondary); font-size: 14px; margin: 10px 0 20px; max-width: 500px; line-height: 1.5;">
                Mantener tu aplicación actualizada te asegura tener las últimas funciones, mejoras de rendimiento y correcciones de seguridad.
            </p>
