...
const MAPA_REEMPLAZO: Record<string, (ctx: ContextoDocumento) => string> = {
    ...
    '[[Estado o Provincia]]': (ctx) => ctx.estado || 'Holguín', // 👇 Ya es dinámico
    ...
    '[[Fecha y hora del ensayo]]': (ctx) => (ctx.ensayo_fecha && ctx.ensayo_fecha !== 'No requiere ensayo previo' && ctx.ensayo_fecha !== '---') ? `${ctx.ensayo_fecha} a las ${ctx.ensayo_hora}` : 'No aplica', // 👇 Condición inteligente
