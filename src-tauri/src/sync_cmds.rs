...
pub struct AsambleaSync {
    pub id: i64, pub tema: String, pub fecha: String, pub identificador: Option<String>,
    pub local_id: Option<i64>, pub ensayo_lugar: Option<String>, pub ensayo_fecha: Option<String>,
    pub ensayo_hora: Option<String>, pub recorridos_info: Option<String>, pub instrucciones_esp: Option<String>,
    pub ensayo_notas: Option<String>, pub jw_stream_studio: i64, pub coordinador_id: Option<i64>,
    pub coordinador_aux_id: Option<i64>, pub prog_super_id: Option<i64>, pub prog_aux_id: Option<i64>,
    pub aloj_super_id: Option<i64>, pub aloj_aux_id: Option<i64>, pub audio_video_super_id: Option<i64>,
    pub video_super_id: Option<i64>, pub audio_super_id: Option<i64>, pub plataforma_super_id: Option<i64>,
    pub bautismo_super_id: Option<i64>, pub bautismo_aux_id: Option<i64>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct LocalSync {
    pub id: i64, pub nombre: String, pub direccion: Option<String>, 
    pub ciudad: Option<String>, pub estado: Option<String>, pub capacidad: Option<i64>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct CongregacionSync {
    pub id: i64, pub asamblea_id: Option<i64>, pub nombre: String, 
    pub circuito: Option<String>, pub numero_congregacion: Option<String>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct PersonaSync {
    pub id: i64, pub asamblea_id: Option<i64>, pub nombre_completo: String, 
    pub sexo: Option<String>, pub privilegios: Option<String>, pub id_congregacion: Option<i64>, 
    pub congregacion: Option<String>, pub telefono: Option<String>, pub email: Option<String>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct ProgramaSync {
    pub id: i64, pub asamblea_id: Option<i64>, pub dia: String, pub sesion: String, 
    pub hora_inicio: Option<String>, pub tema: String, pub tipo: Option<String>, 
    pub duracion: Option<i64>, pub orador_id: Option<i64>, pub es_video: bool, 
    pub estado: Option<String>, pub esta_presente: bool, pub numero_bosquejo: Option<String>, 
    pub ensayo_terminado: bool, pub fuente: Option<String>, pub es_betelita: bool, 
    pub es_interprete: bool, pub es_visitante: bool,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct AsignacionSync {
    pub id: i64, pub asamblea_id: Option<i64>, pub dia: String, pub fecha: Option<String>,
    pub tipo_asignacion: String, pub persona_id: i64, pub estado: Option<String>, 
    pub esta_presente: bool, pub ensayo_terminado: bool,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct PlantillaCartaSync {
    pub id: String, pub contenido_html: String,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct PlantillaEmailSync {
    pub id: String, pub asunto: Option<String>, pub cuerpo: Option<String>,
}

#[derive(Serialize, Deserialize, Debug)]
pub struct ConfiguracionSync {
    pub id: i64, pub nombre: Option<String>, pub segundo_nombre: Option<String>,
    pub apellido: Option<String>, pub sufijo: Option<String>, pub email: Option<String>,
    pub email_jwpub: Option<String>, pub movil: Option<String>, pub identificador: Option<String>,
    pub fecha_creacion: Option<String>, pub tema: Option<String>, pub idioma: Option<String>,
...
pub fn obtener_last_sync_local(db_state: State<DbState>) -> Result<Option<String>, String> {
    ...
    let mut stmt = conn.prepare("SELECT last_synced_at FROM configuracion WHERE id = 1").map_err(|e| e.to_string())?;
...
pub fn actualizar_last_sync_local(fecha: String, db_state: State<DbState>) -> Result<(), String> {
    let conn = db_state.conn.lock().unwrap();
    let _ = conn.execute("ALTER TABLE configuracion ADD COLUMN last_synced_at TEXT", []);
    conn.execute("UPDATE configuracion SET last_synced_at = ?1 WHERE id = 1", params![fecha]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn importar_db_json(json_data: String, db_state: State<DbState>) -> Result<(), String> {
    ...
    let backup: BackupData = serde_json::from_str(&json_data).map_err(|e| format!("Error deserializando JSON: {}", e))?;
    ...
    // Vaciamos TODO
    tx.execute("DELETE FROM asignaciones_especiales", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM programa", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM personas", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM congregaciones", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM locales", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM asambleas", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM plantillas_cartas", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM plantillas_email", []).map_err(|e| e.to_string())?;
    tx.execute("DELETE FROM configuracion", []).map_err(|e| e.to_string())?;
    ...
    for p in backup.plantillas_cartas {
        tx.execute("INSERT INTO plantillas_cartas (id, contenido_html) VALUES (?1, ?2)", params![p.id, p.contenido_html]).map_err(|e| e.to_string())?;
    }
    for p in backup.plantillas_email {
        tx.execute("INSERT INTO plantillas_email (id, asunto, cuerpo) VALUES (?1, ?2, ?3)", params![p.id, p.asunto, p.cuerpo]).map_err(|e| e.to_string())?;
    }
...
fn extraer_configuracion(conn: &Connection) -> Result<Vec<ConfiguracionSync>, String> {
    let _ = conn.execute("ALTER TABLE configuracion ADD COLUMN last_synced_at TEXT", []); // Seguro
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(ConfiguracionSync {
            id: row.get(0)?, nombre: row.get(1)?, segundo_nombre: row.get(2)?, apellido: row.get(3)?, sufijo: row.get(4)?,
            email: row.get(5)?, email_jwpub: row.get(6)?, movil: row.get(7)?, identificador: row.get(8)?,
            fecha_creacion: row.get(9)?, tema: row.get(10)?, idioma: row.get(11)?, last_synced_at: row.get(12)?,
        })
    }).unwrap();
...
fn extraer_plantillas_cartas(conn: &Connection) -> Result<Vec<PlantillaCartaSync>, String> {
    let mut stmt = conn.prepare("SELECT id, contenido_html FROM plantillas_cartas").unwrap();
    let map_iter = stmt.query_map([], |row| Ok(PlantillaCartaSync { id: row.get(0)?, contenido_html: row.get(1)? })).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}

fn extraer_plantillas_email(conn: &Connection) -> Result<Vec<PlantillaEmailSync>, String> {
    let mut stmt = conn.prepare("SELECT id, asunto, cuerpo FROM plantillas_email").unwrap();
    let map_iter = stmt.query_map([], |row| Ok(PlantillaEmailSync { id: row.get(0)?, asunto: row.get(1)?, cuerpo: row.get(2)? })).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_asambleas(conn: &Connection) -> Result<Vec<AsambleaSync>, String> {
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(AsambleaSync {
            id: row.get(0)?, tema: row.get(1)?, fecha: row.get(2)?, identificador: row.get(3)?, local_id: row.get(4)?,
            ensayo_lugar: row.get(5)?, ensayo_fecha: row.get(6)?, ensayo_hora: row.get(7)?, recorridos_info: row.get(8)?,
            instrucciones_esp: row.get(9)?, ensayo_notas: row.get(10)?, jw_stream_studio: row.get(11)?, coordinador_id: row.get(12)?,
            coordinador_aux_id: row.get(13)?, prog_super_id: row.get(14)?, prog_aux_id: row.get(15)?, aloj_super_id: row.get(16)?,
            aloj_aux_id: row.get(17)?, audio_video_super_id: row.get(18)?, video_super_id: row.get(19)?, audio_super_id: row.get(20)?,
            plataforma_super_id: row.get(21)?, bautismo_super_id: row.get(22)?, bautismo_aux_id: row.get(23)?,
        })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_locales(conn: &Connection) -> Result<Vec<LocalSync>, String> {
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(LocalSync { id: row.get(0)?, nombre: row.get(1)?, direccion: row.get(2)?, ciudad: row.get(3)?, estado: row.get(4)?, capacidad: row.get(5)? })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_congregaciones(conn: &Connection) -> Result<Vec<CongregacionSync>, String> {
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(CongregacionSync { id: row.get(0)?, asamblea_id: row.get(1)?, nombre: row.get(2)?, circuito: row.get(3)?, numero_congregacion: row.get(4)? })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_personas(conn: &Connection) -> Result<Vec<PersonaSync>, String> {
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(PersonaSync {
            id: row.get(0)?, asamblea_id: row.get(1)?, nombre_completo: row.get(2)?, sexo: row.get(3)?, privilegios: row.get(4)?,
            id_congregacion: row.get(5)?, congregacion: row.get(6)?, telefono: row.get(7)?, email: row.get(8)?
        })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_programa(conn: &Connection) -> Result<Vec<ProgramaSync>, String> {
    ...
    let map_iter = stmt.query_map([], |row| {
        Ok(ProgramaSync {
            id: row.get(0)?, asamblea_id: row.get(1)?, dia: row.get(2)?, sesion: row.get(3)?, hora_inicio: row.get(4)?,
            tema: row.get(5)?, tipo: row.get(6)?, duracion: row.get(7)?, orador_id: row.get(8)?, es_video: row.get(9)?,
            estado: row.get(10)?, esta_presente: row.get(11)?, numero_bosquejo: row.get(12)?, ensayo_terminado: row.get(13)?,
            fuente: row.get(14)?, es_betelita: row.get(15)?, es_interprete: row.get(16)?, es_visitante: row.get(17)?
        })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
...
fn extraer_asignaciones(conn: &Connection) -> Result<Vec<AsignacionSync>, String> {
    let mut stmt = conn.prepare("SELECT * FROM asignaciones_especiales").unwrap();
    let map_iter = stmt.query_map([], |row| {
        Ok(AsignacionSync {
            id: row.get(0)?, asamblea_id: row.get(1)?, dia: row.get(2)?, fecha: row.get(3)?, tipo_asignacion: row.get(4)?,
            persona_id: row.get(5)?, estado: row.get(6)?, esta_presente: row.get(7)?, ensayo_terminado: row.get(8)?
        })
    }).unwrap();
    Ok(map_iter.filter_map(Result::ok).collect())
}
