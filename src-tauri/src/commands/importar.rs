...
pub fn importar_congregaciones_csv(
    ...
    let mut stmt_update = tx
        .prepare("UPDATE congregaciones SET numero_congregacion = ?1, circuito = ?2 WHERE id = ?3")
    ...
    for result in rdr.deserialize() {
        ...
        let existe: Option<i32> = stmt_check
            .query_row(params![asamblea_id, nombre], |row| row.get(0))
            .optional()
            .unwrap_or(None);

        if let Some(id) = existe {
            stmt_update
                .execute(params![numero, circuito, id])
                .unwrap_or(0);
            actualizadas += 1;
        } else {
            stmt_insert
                .execute(params![asamblea_id, nombre, numero, circuito])
                .unwrap_or(0);
            nuevas += 1;
        }
    }
    ...
    Ok(format!(
        "✅ {} nuevas, {} actualizadas en esta asamblea.",
        nuevas, actualizadas
    ))
}
...
pub fn importar_personas_csv(
    ...
    for result in rdr.deserialize() {
        ...
        let nombre_final = if fila.nombre.is_some() {
            ...
            if let Some((ap, nom)) = nc.split_once(',') {
        ...
        if let Some(cong) = &fila.congregacion {
            if !cong.trim().is_empty() {
                let existe: Option<i32> = stmt_find_cong
                    .query_row(params![asamblea_id, cong.trim()], |row| row.get(0))
                    .optional()
                    .unwrap_or(None);
                if let Some(cid) = existe {
                    id_cong = cid;
                } else {
                    stmt_ins_cong
        ...
        let existe_p: Option<i32> = stmt_check_pers
            .query_row(params![asamblea_id, nombre_final.clone()], |row| row.get(0))
            .optional()
            .unwrap_or(None);
    ...
    drop(stmt_find_cong);
    drop(stmt_ins_cong);
    drop(stmt_check_pers);
    drop(stmt_ins_pers);
    drop(stmt_upd_pers);
    tx.commit().map_err(|e| e.to_string())?;
    Ok(format!(
        "✅ {} personas procesadas para esta asamblea.",
        contador
    ))
}
...
pub fn importar_programa_jw(
    ...
    for result in rdr.deserialize() {
        ...
        let sesion = if hora_limpia.as_str() < "13:00" {
            "Mañana"
        } else {
            "Tarde"
        };
        ...
        let tipo = if es_video {
            "Video"
        } else if titulo_l.contains("canción") || titulo_l.contains("cántico") {
            "Cántico"
        } else if titulo_l.contains("simposio") {
            "Simposio"
        } else if titulo_l.contains("lectura") {
            "Lectura Bíblica"
        } else {
            "Discurso"
        };
        ...
        if !es_video && fila.orador.is_some() {
            ...
            if !raw.trim().is_empty() {
                let nombre_final = if let Some((ap, nom)) = raw.split_once(',') {
                    format!("{} {}", nom.trim(), ap.trim())
                } else {
                    raw.clone()
                };
                ...
                // Buscamos persona EN ESTA ASAMBLEA
                let existe: Option<i32> = stmt_find_pers
                    .query_row(params![asamblea_id, nombre_final.clone()], |row| row.get(0))
                    .optional()
                    .unwrap_or(None);
                ...
                if let Some(id) = existe {
                    ...
                    if let Some(cong) = &fila.congregacion {
                        if !cong.trim().is_empty() {
                            // Buscamos congregación EN ESTA ASAMBLEA
                            let ex_cong: Option<i32> = stmt_find_cong
                                .query_row(params![asamblea_id, cong.trim()], |row| row.get(0))
                                .optional()
                                .unwrap_or(None);
                            if let Some(cid) = ex_cong {
                                id_cong = cid;
                            } else {
                                stmt_ins_cong
                                    .execute(params![asamblea_id, cong.trim()])
                    ...
                    stmt_ins_pers
                        .execute(params![asamblea_id, nombre_final, id_cong, tel, email])
                        .unwrap_or(0);
