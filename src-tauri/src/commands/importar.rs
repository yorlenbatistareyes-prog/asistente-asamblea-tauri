...
// ==========================================
// 3. COMANDOS (AJUSTADOS CON ASAMBLEA_ID)
// ==========================================

#[command]
pub fn importar_congregaciones_csv(app: AppHandle, asamblea_id: i32, ruta_archivo: String) -> Result<String, String> {
    let mut rdr = preparar_lector(&ruta_archivo)?;
    let mut conn = conectar_db(&app);
    let tx = conn.transaction().map_err(|e| e.to_string())?;

    // Buscamos solo en la asamblea actual
    ...
    for result in rdr.deserialize() {
        ...
        let existe: Option<i32> = stmt_check.query_row(params![asamblea_id, nombre], |row| row.get(0)).optional().unwrap_or(None);

        if let Some(id) = existe {
            stmt_update.execute(params![numero, circuito, id]).unwrap_or(0);
            actualizadas += 1;
        } else {
            stmt_insert.execute(params![asamblea_id, nombre, numero, circuito]).unwrap_or(0);
            nuevas += 1;
        }
    }
    drop(stmt_check); drop(stmt_update); drop(stmt_insert);
    tx.commit().map_err(|e| e.to_string())?;
    Ok(format!("✅ {} nuevas, {} actualizadas en esta asamblea.", nuevas, actualizadas))
}

#[command]
pub fn importar_personas_csv(app: AppHandle, asamblea_id: i32, ruta_archivo: String) -> Result<String, String> {
    let mut rdr = preparar_lector(&ruta_archivo)?;
    let mut conn = conectar_db(&app);
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    ...
    // Si la creamos al vuelo, la vinculamos a esta asamblea
    ...
    let mut stmt_ins_pers = tx.prepare("INSERT INTO personas (asamblea_id, nombre_completo, genero, privilegios, id_congregacion, telefono, email) VALUES (?1, ?2, 'Hombre', ?3, ?4, ?5, ?6)").map_err(|e| e.to_string())?;
    ...
    for result in rdr.deserialize() {
        ...
        if let Some(cong) = &fila.congregacion {
            if !cong.trim().is_empty() {
                let existe: Option<i32> = stmt_find_cong.query_row(params![asamblea_id, cong.trim()], |row| row.get(0)).optional().unwrap_or(None);
        ...
        let existe_p: Option<i32> = stmt_check_pers.query_row(params![asamblea_id, nombre_final.clone()], |row| row.get(0)).optional().unwrap_or(None);
    ...
    tx.commit().map_err(|e| e.to_string())?;
    Ok(format!("✅ {} personas procesadas para esta asamblea.", contador))
}

#[command]
pub fn importar_programa_jw(app: AppHandle, asamblea_id: i32, ruta_archivo: String) -> Result<String, String> {
    let mut rdr = preparar_lector(&ruta_archivo)?;
    let mut conn = conectar_db(&app);
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    
    // Todas las búsquedas e inserciones filtradas por asamblea_id
    let mut stmt_find_cong = tx.prepare("SELECT id FROM congregaciones WHERE asamblea_id = ?1 AND LOWER(TRIM(nombre)) = LOWER(TRIM(?2))").map_err(|e| e.to_string())?;
    let mut stmt_ins_cong = tx.prepare("INSERT INTO congregaciones (asamblea_id, nombre, numero_congregacion) VALUES (?1, ?2, '')").map_err(|e| e.to_string())?;
    ...
    let mut stmt_ins_pers = tx.prepare("INSERT INTO personas (asamblea_id, nombre_completo, genero, privilegios, id_congregacion, telefono, email) VALUES (?1, ?2, 'Hombre', 'Orador', ?3, ?4, ?5)").map_err(|e| e.to_string())?;
    
    // Insertamos programa vinculado a esta asamblea
    ...
    for result in rdr.deserialize() {
        ...
        if !es_video && fila.orador.is_some() {
            ...
            if !raw.trim().is_empty() {
                ...
                // Buscamos persona EN ESTA ASAMBLEA
                let existe: Option<i32> = stmt_find_pers.query_row(params![asamblea_id, nombre_final.clone()], |row| row.get(0)).optional().unwrap_or(None);
                ...
                if let Some(id) = existe { orador_id = Some(id); } 
                    ...
                    if let Some(cong) = &fila.congregacion {
                        if !cong.trim().is_empty() {
                            // Buscamos congregación EN ESTA ASAMBLEA
                            let ex_cong: Option<i32> = stmt_find_cong.query_row(params![asamblea_id, cong.trim()], |row| row.get(0)).optional().unwrap_or(None);
                    ...
                    stmt_ins_pers.execute(params![asamblea_id, nombre_final, id_cong, tel, email]).unwrap_or(0);
    ...
    tx.commit().map_err(|e| e.to_string())?;
    Ok("Programa importado exitosamente a esta asamblea".to_string())
}
